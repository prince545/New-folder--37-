...
export default function FindPivotIndex({ approach = "optimal" }) {
    const arr = [1, 7, 3, 6, 5, 6];
    const [idx, setIdx] = useState(0);
    const total = arr.reduce((a, b) => a + b, 0);
    let left = 0;
    const results = arr.map((v) => { const r = { left, right: total - left - v, val: v }; left += v; return r; });
    const r = results[idx];
    ...
    return (
        <div style={{ fontFamily: "monospace" }}>
            <div style={{ color: "#94a3b8", fontSize: 13, marginBottom: 12 }}>
                Find index where leftSum == rightSum. Total={total}
            </div>
            <div style={{ display: "flex", gap: 6, marginBottom: 16 }}>
                {arr.map((v, i) => (
                    <div key={i} onClick={() => setIdx(i)} style={{
                        ...
                        background: i === idx ? (isPivot ? "#14532d" : "#6366f1") : "#0f172a",
                        border: `2px solid ${i === idx ? (isPivot ? "#4ade80" : "#818cf8") : "#334155"}`,
            ...
            <div style={{ color: "#64748b", fontSize: 12, marginBottom: 12 }}>
                KEY INSIGHT: rightSum = total − leftSum − arr[i]. No extra array needed.
            </div>
            <div style={{ display: "flex", gap: 8 }}>
                ...
                <button onClick={() => setIdx(0)} style={{ ...btnStyle, background: "#1e293b" }}>Reset</button>
            </div>
        </div>
    );
}
