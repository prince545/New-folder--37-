...
export default function Problem200({ approach = "optimal" }) {
    ...
    // BRUTE FORCE LOGIC
    const stepsBrute = [
        { desc: "Brute Force approach for Number of Islands", data: [1, 2, 3, 4], pointers: { i: 0 }, scalars: { approach: "Brute Force O(N^2) or worse" } },
        { desc: "Nested loops processing...", data: [1, 2, 3, 4], pointers: { i: 0, j: 1 }, scalars: { approach: "Brute Force" } }
    ];

    // BETTER LOGIC
    const stepsBetter = [
        { desc: "Better approach for Number of Islands", data: [1, 2, 3, 4], pointers: { i: 0 }, scalars: { approach: "Better (e.g. O(N log N))" } },
        { desc: "Optimizing state...", data: [1, 2, 3, 4], pointers: { i: 1 }, scalars: { approach: "Better" } }
    ];

    // OPTIMAL LOGIC
    const stepsOptimal = [
        { desc: "Optimal approach for Number of Islands", data: [1, 2, 3, 4], pointers: { left: 0, right: 3 }, scalars: { approach: "Optimal O(N)" } },
