...
// Read problems
const problemsData = JSON.parse(fs.readFileSync('./src/data/problems.json', 'utf-8'));
...
problemsData.forEach(p => {
    ...
    const isGridProblem = p.category.toLowerCase().includes("graph") || p.category.toLowerCase().includes("2d dp") || p.category.toLowerCase().includes("matrix") || p.category.toLowerCase().includes("island");

    const placeholderData = isGridProblem ? '[[1, 2], [3, 4]]' : '[1, 2, 3, 4]';
    const initialPointer = isGridProblem ? '{ r: 0, c: 0 }' : '{ i: 0 }';
    const secondaryPointer = isGridProblem ? '{ r: 0, c: 1 }' : '{ i: 0, j: 1 }';
    const optimalPointer1 = isGridProblem ? '{ r: 0, c: 0 }' : '{ left: 0, right: 3 }';
    const optimalPointer2 = isGridProblem ? '{ r: 1, c: 1 }' : '{ left: 1, right: 2 }';
...
export default function ${componentName}({ approach = "optimal" }) {
    ...
    // BRUTE FORCE LOGIC
    const stepsBrute = [
        { desc: "Brute Force approach for ${p.title}", data: ${placeholderData}, pointers: ${initialPointer}, scalars: { approach: "Brute Force O(N^2) or worse" } },
        { desc: "Nested loops processing...", data: ${placeholderData}, pointers: ${secondaryPointer}, scalars: { approach: "Brute Force" } }
    ];

    // BETTER LOGIC
    const stepsBetter = [
        { desc: "Better approach for ${p.title}", data: ${placeholderData}, pointers: ${initialPointer}, scalars: { approach: "Better (e.g. O(N log N))" } },
        { desc: "Optimizing state...", data: ${placeholderData}, pointers: ${secondaryPointer}, scalars: { approach: "Better" } }
    ];

    // OPTIMAL LOGIC
    const stepsOptimal = [
        { desc: "Optimal approach for ${p.title}", data: ${placeholderData}, pointers: ${optimalPointer1}, scalars: { approach: "Optimal O(N)" } },
        { desc: "Directly solving...", data: ${placeholderData}, pointers: ${optimalPointer2}, scalars: { approach: "Optimal" } }
    ];
